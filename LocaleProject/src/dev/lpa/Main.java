...
//        Locale enInvalid = new Locale("bx", "BX"); // no error as format correct
...
//        Locale enInvalidTwo = new Locale.Builder().setLanguage("bx").setRegion("BX").build(); // no error as format correct
...
//            System.out.print(String.format(locale, "\t%1$tA, %1$tB %1$te, %1$tY %n", May5)); // same as below
            System.out.printf(locale, "\t%1$tA, %1$tB %1$te, %1$tY %n", May5);

            NumberFormat decimalInfo = NumberFormat.getNumberInstance(locale);
            decimalInfo.setMaximumFractionDigits(6); // default decimal place is 3, now change to 6
...
//        BigDecimal myLoan = new BigDecimal("1,000.50");
        ...
        BigDecimal myLoan = scanner.nextBigDecimal(); // the format of the input need to match the format of locale
