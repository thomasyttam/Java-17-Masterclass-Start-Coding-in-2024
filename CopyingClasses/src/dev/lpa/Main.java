...
record Person (String name, String dob, Person[] kids) {
    ...
    public Person(Person p) {
        this(p.name, p.dob,
                p.kids == null ? null : Arrays.copyOf(p.kids, p.kids.length)); // create deep copy of array
    }
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //copy of the array of same reference of the element
//        Person[] personsCopy = Arrays.copyOf(persons, persons.length);
//        Person[] personsCopy = persons.clone(); //-> create shallow copy only
        Person[] personsCopy = new Person[5];

//        For below code, create a new array and copy all elements to new array


/*      Not using copy constructor
        for (int i = 0; i < 5; i++) {
            Person current = persons[i];
//            var kids = current.kids(); // same reference of the elements as kids is array
            var kids = current.kids() == null ? null :
                    Arrays.copyOf(current.kids(), current.kids().length); // copy of elements
...
//        }
        // setAll method replace above for loop
