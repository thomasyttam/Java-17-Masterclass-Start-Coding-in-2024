...
public class MainFinalChallenge {
    ...
    public static void main(String[] args) {
        ...
        Course cplusplus = new Course("CI","C++ Introduction", 200);

        int currentYear = LocalDate.now().getYear();
        List<Student> students = Stream
                .generate(() -> Student.getRandomStudent(jmc, pymc, jgames,cplusplus))
                .filter(s -> s.getYearEnrolled() >= (currentYear - 4)) // enrolled within 4 years
                .limit(10000)
                .toList();
        ...
        var percentagesSum = students.stream()
                .flatMap(s -> s.getEngagementMap().values().stream())
                .collect(Collectors.groupingBy(CourseEngagement::getCourseCode,
                        Collectors.summarizingDouble(CourseEngagement::getPercentComplete)));

        percentagesSum.forEach((k, v) -> System.out.println(k + " " + v));
